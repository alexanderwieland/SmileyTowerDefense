...
namespace SmileyTowerDefense
{
    ...
    public class Pathfinder
    {
        ...
        private SearchNode[,] searchNodes;  //google it when network not overloaded
        ...
        public void InitializeSearchNodes()
        {
            ...
            for (int x = 0; x < levelWidth; x++)
            {
                for (int y = 0; y < levelHeight; y++)
                {
                    ...
                    node.Walkable = map.GetIndex(y, x) == 0;        //TODOne negate: now 1 means walkable; TODOne NO! IT'S LIKE ORIGINALLY THOUGH! walk-flag
                    ...
                    {
                        // todone no Neighbor-nodes needed depending on FindPath method. oh yes they are-downside of algo implementation
                        node.Neighbors = new SearchNode[4];
...
#if DEBUG
                    //if (smallestDistanceToGoal > 21.0)
        ...
        public List<Vector2> FindPath(Point startPoint, Point endPoint)
        {
            ...
            while (openList.Count > 0)
            {
                ...
                for (int i = 0; i < currentNode.Neighbors.Length; i++)
                {
                    ...
                    //Neigbors[0] means neighbor above current node, [1] = underneath, [2] = left, [3] = right
                    ...
                    else if (neighbor.InOpenList || neighbor.InClosedList)
                    {
                        ...
                        //     the Open List again.
                        if (neighbor.DistanceTraveled > distanceTraveled)   //= true when current path is faster than the other option to get to the neighbor node :)
                        {
                            neighbor.DistanceTraveled = distanceTraveled;
                            neighbor.DistanceToGoal = distanceTraveled + heuristic;
